...
async function getScopedStudentIds(req) {
    ...
    if (role === 'teacher') {
        const students = await Student_1.Student.find({ teacherId: req.user.userId, isDeleted: false }).select('_id').lean();
...
async function createAttendanceRecord(req, res, next) {
    try {
        ...
        const existing = await Attendance_1.Attendance.findOne({
            ...
            ...(req.body.timetableId ? { timetableId: req.body.timetableId } : { attendanceDate, session: req.body.session }),
            isDeleted: false
        }).lean();
        if (existing) {
            return res.status(409).json((0, response_1.createError)('Attendance already recorded for this student, date, and session'));
        }
        ...
        const attendance = await Attendance_1.Attendance.create({
            ...
            checkInAt: req.body.status === 'absent' ? null : new Date(),
        ...
        const savedAttendance = await Attendance_1.Attendance.findById(attendance._id)
            ...
            .populate('teacherId', 'name email')
            .lean();
        res.status(201).json((0, response_1.createResponse)(serializeAttendance(savedAttendance), 'Attendance recorded successfully'));
    }
    catch (error) {
        next(error);
    }
