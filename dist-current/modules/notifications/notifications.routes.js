...
router.get('/public', (0, validate_1.validate)(notificationQuerySchema), async (req, res, next) => {
    try {
        ...
        const [notifications, total] = await Promise.all([
            Notification_1.Notification.find(baseFilter)
                ...
                .skip((page - 1) * limit)
                .limit(limit),
            Notification_1.Notification.countDocuments(baseFilter)
        ]);
...
router.get('/', (0, auth_1.authorize)([...readRoles]), (0, validate_1.validate)(notificationQuerySchema), async (req, res, next) => {
    try {
        ...
        const search = String(req.query.search || '').trim();
        const filter = applySearch(isManagementViewer(req) ? buildManagementFilter(req) : buildAudienceFilter(req, true), search);
        const lang = resolveRequestLang(req);
        const [notifications, total] = await Promise.all([
...
const updateNotificationHandler = async (req, res, next) => {
    try {
        ...
        const existingNotification = await Notification_1.Notification.findOne({
