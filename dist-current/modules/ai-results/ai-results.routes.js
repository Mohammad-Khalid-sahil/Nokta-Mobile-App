...
router.get('/result/:resultId', (0, auth_1.authorize)(['super_admin', 'admin', 'owner', 'branch_manager', 'teacher', 'student', 'parent', 'family_student']), (0, validate_1.validate)(resultParamsSchema), async (req, res, next) => {
    try {
        ...
        const insight = await AIResultInsight_1.AIResultInsight.findOne({
            studentId: result.student,
            examId: result.exam,
            isDeleted: false
        }).lean();
        return res.json((0, response_1.createResponse)(insight ?? null));
    }
    catch (error) {
        next(error);
    }
...
router.post('/generate/:resultId', rateLimiter_1.authLimiter, (0, auth_1.authorize)(['super_admin', 'admin', 'branch_manager', 'teacher']), (0, validate_1.validate)(resultParamsSchema), async (req, res, next) => {
    try {
        ...
        const insight = await (0, aiResultAnalysis_service_1.upsertAIResultInsight)({ resultId: req.params.resultId, actorId: req.user?.userId ?? null });
