...
function teacherCanTeachClassSubject(teacher, klass, subject) {
    const assignedSubjectIds = Array.isArray(teacher?.assignedSubjects) ? teacher.assignedSubjects.map((item) => String(item?._id ?? item)) : [];
    const assignedClassIds = Array.isArray(teacher?.assignedClasses) ? teacher.assignedClasses.map((item) => String(item?._id ?? item)) : [];
    const classTeacherIds = Array.isArray(klass?.assignedTeachers) ? klass.assignedTeachers.map((item) => String(item?._id ?? item)) : [];
    return ((subject?.teacher && String(subject.teacher) === String(teacher?._id)) ||
        assignedSubjectIds.includes(String(subject?._id)) ||
        (assignedClassIds.includes(String(klass?._id)) && (!assignedSubjectIds.length || assignedSubjectIds.includes(String(subject?._id)))) ||
        (classTeacherIds.includes(String(teacher?._id)) && (!assignedSubjectIds.length || assignedSubjectIds.includes(String(subject?._id)))));
}
