...
assert(!permissionService.hasRequiredAccess({ role: 'student' }, generatePolicy), 'student cannot access AI generate route policy');
assert(!permissionService.hasRequiredAccess({ role: 'parent' }, generatePolicy), 'parent cannot access AI generate route policy');
assert(permissionService.hasRequiredAccess({ role: 'admin' }, generatePolicy), 'admin can access AI generate route policy');
...
assert(!permissionService.hasRequiredAccess({ role: 'admin' }, branchDeletePolicy), 'admin cannot delete branches via route policy');
assert(!permissionService.hasRequiredAccess({ role: 'branch_manager' }, branchDeletePolicy), 'branch_manager cannot delete branches via route policy');
...
assert(!permissionService.hasRequiredAccess({ role: 'admin' }, branchCreatePolicy), 'admin cannot create branches via route policy');
