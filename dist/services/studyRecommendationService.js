...
exports.buildStudyPlanFromAnalysis = buildStudyPlanFromAnalysis;
exports.buildStudyRecommendation = buildStudyRecommendation;
...
function subjectFocus(subjectName) {
...
function copyFor(language) {
    if (language === 'fa') {
        return {
            reasonLow: (score, subject) => `نمره ${score} در ${subject} — نیاز به تقویت مباحث ضعیف.`,
            reasonGood: (score, subject) => `نمره ${score} در ${subject} — تمرین هدفمند و مرور توصیه می‌شود.`,
            reasonHigh: (score, subject) => `نمره ${score} در ${subject} — آماده محتوای پیشرفته.`,
            failedLow: (missed, subject) => `حدود ${missed}% محتوای آزمون ${subject} نیاز به بازبینی دارد.`,
    ...
    if (language === 'ps') {
        return {
            reasonLow: (score, subject) => `نمره ${score} په ${subject} کې — ضعیف موضوعات باید پیاوړي شي.`,
            reasonGood: (score, subject) => `نمره ${score} په ${subject} کې — هدفمند تمرین او بیاکتنه سپارښتنه کېږي.`,
            reasonHigh: (score, subject) => `نمره ${score} په ${subject} کې — د پرمختللي موادو لپاره چمتو.`,
            failedLow: (missed, subject) => `شاوخوا ${missed}% د ${subject} ازموینې محتوا بیاکتنې ته اړتیا لري.`,
    ...
    return {
        reasonLow: (score, subject) => `Score ${score} in ${subject} — strengthen weak topics.`,
        reasonGood: (score, subject) => `Score ${score} in ${subject} — targeted practice and revision recommended.`,
        reasonHigh: (score, subject) => `Score ${score} in ${subject} — ready for advanced content.`,
        failedLow: (missed, subject) => `About ${missed}% of ${subject} exam content needs review.`,
...
function parseWeakTopics(remarks, provided = []) {
 ...
 */
function buildStudyPlanFromAnalysis(input) {
    ...
    const band = (0, aiInsightService_1.classifyPerformanceBand)(score);
    ...
    if (lowScoreSubjects.length) {
        recommendations.push(...lowScoreSubjects.slice(0, 2).map((subject) => `${copy.practice}: ${subject}`));
    }
    return {
        title,
        message: title,
...
async function loadCatalogResources(params) {
    ...
    const resourceFilter = { published: true, isDeleted: { $ne: true } };
    if (subjectId)
    ...
    const bookFilter = { available: true, isDeleted: false };
    if (subjectName) {
        bookFilter.$or = [
    ...
    if (subjectId)
        courseFilter.subjects = subjectId;
    const [learningResources, books, courses] = await Promise.all([
        LearningResource_1.LearningResource.find(resourceFilter)
            ...
            .limit(8)
            .lean(),
        Book_1.Book.find(bookFilter)
            .select('title author category description fileUrl coverUrl')
            .sort({ updatedAt: -1 })
            .limit(4)
            .lean(),
        Course_1.Course.find(courseFilter)
    ...
    const mappedCourses = courses.map((item) => ({
        ...
        description: pickLocalized(item.shortDescription, language) || pickLocalized(item.description, language),
        url: '',
        actionLabel: copy.open
    }));
    const lessons = [];
    const exercises = [];
    const videosAsCourses = [];
    for (const item of learningResources) {
        ...
        const resource = {
            ...
            kindLabel: kindLabel(kind === 'video' ? 'video' : kind === 'exercise' ? 'exercise' : kind === 'book' ? 'book' : 'lesson', copy),
            title: String(item.title ?? ''),
        ...
        if (kind === 'book')
            mappedBooks.push(resource);
        else if (kind === 'exercise')
            exercises.push(resource);
        else if (kind === 'video')
            videosAsCourses.push(resource);
        else
...
async function loadLowScoreSubjects(studentId, currentResultId) {
    if (!studentId)
        return [];
    const previous = await Result_1.Result.find({
        student: studentId,
 ...
 */
async function buildStudyRecommendation(params) {
