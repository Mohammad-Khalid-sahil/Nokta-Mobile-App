...
function datePart(field, operator) {
...
async function buildDashboardSummary(req, res, next) {
    try {
        ...
        const [totalStudents, totalTeachers, totalClasses, totalSubjects, totalBranches, totalBooks, totalNotifications, totalUsers, totalFamilies, paymentTotals, manualIncomeTotals, expenseTotals, studentBalance, totalAuditLogs, studentTrend, teacherTrend, monthlyPayments, monthlyManualIncome, monthlyExpenses, expenseCategoryBreakdown, totalAttendance, totalExams, totalResultsCount, totalPayments, totalFinanceEntries, totalReports, totalMessages, totalCourses, totalCurriculum, totalRoles] = await Promise.all([
            ...
            isTeacher || isStudent || isFamily ? Promise.resolve(0) : Course_1.Course.countDocuments({ isDeleted: false, ...periodCreatedAt }),
            isTeacher || isStudent || isFamily ? Promise.resolve(0) : Curriculum_1.Curriculum.countDocuments({ isDeleted: false, ...periodCreatedAt }),
            isTeacher || isStudent || isFamily ? Promise.resolve(0) : Role_1.Role.countDocuments({})
        ]);
        ...
        const cashFlowSignal = monthlyFinances.reduce((sum, row) => sum + (Number(row.income) || 0) + (Number(row.expenses) || 0), 0);
        const enrollmentSignal = enrollmentTrend.reduce((sum, row) => sum + (Number(row.students) || 0) + (Number(row.teachers) || 0), 0);
        logger_1.logger.info('Admin dashboard analytics', {
        ...
        logger_1.logger.error('Admin dashboard analytics failed', error, {
            requestUrl: req.originalUrl || req.url,
