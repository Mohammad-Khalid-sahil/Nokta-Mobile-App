...
const aiResultAnalysis_service_1 = require("../ai-results/aiResultAnalysis.service");
...
const maxItems = 100;
const learningResourceUploadRoot = path_1.default.resolve(process.cwd(), 'uploads', 'learning-resources');
...
if (!fs_1.default.existsSync(learningResourceUploadRoot)) {
    fs_1.default.mkdirSync(learningResourceUploadRoot, { recursive: true });
}
const learningResourceUpload = (0, multer_1.default)({
...
function isPendingScheduleText(value) {
...
function resolveClassScheduleSlots(item, timetableByClass) {
    ...
    return weekly
        ...
        .map((slot) => scheduleDto({ ...slot, room: slot.room ?? item.room, classId: item }, item));
}
function pickPrimaryScheduleSlot(slots) {
    if (!slots.length)
        return null;
    ...
    if (upcoming.length) {
        ...
        const laterToday = sorted.find((slot) => slot.dayOfWeek === today &&
            (timeToMinutes(slot.startTime) ?? 0) >= nowMinutes);
        return laterToday ?? sorted[0];
    }
    return slots[0];
}
function classScheduleSummary(slots) {
    if (!slots.length)
        return '';
    ...
    if (slots.length > 1)
        parts.push(`+${slots.length - 1}`);
    return parts.join(' · ');
}
function classSchedule(item, timetableByClass) {
...
function nextScheduleDate(dayOfWeek) {
...
function scheduleStatus(dayOfWeek, startTime, endTime) {
    const target = Number(dayOfWeek);
    const start = timeToMinutes(startTime);
    const end = timeToMinutes(endTime);
    if (!Number.isFinite(target) || start == null || end == null)
        return 'upcoming';
    const today = kabulDayOfWeek();
    if (today !== target)
        return 'upcoming';
...
function scheduleDto(entry, fallbackClass) {
...
function pickLocalizedValue(value, language = 'en') {
...
function courseDto(item, language = 'en', enrollmentStatus = '') {
    ...
    return {
        ...
        subjectNames: Array.isArray(item.subjects)
            ? item.subjects.map((subject) => subject?.title ?? subject?.name ?? subject?.subjectName).filter(Boolean).join(', ')
            : '',
        status: item.status ?? ''
    };
}
function teacherCourseDto(item, language = 'en', options) {
...
function classDto(item, options) {
    ...
    const subjectNames = subjects
        .map((subject) => subject?.title ?? subject?.name ?? subject?.subjectName)
        .filter(Boolean)
        .join(', ');
    const primaryImage = item.imageUrl ?? item.thumbnailUrl ?? (Array.isArray(item.galleryImages) ? item.galleryImages[0] : '');
    const resolvedStudentCount = Number(item.enrolledStudentCount ?? item.studentCount ?? item.students ?? 0);
...
async function activeStudentBelongsToClass(studentDocId, classId) {
...
async function activeStudentCountsByClass(classIds) {
    if (!classIds.length)
        return new Map();
    const counts = new Map(classIds.map((id) => [String(id), 0]));
    const [directRows, enrollmentRows] = await Promise.all([
        ...
        Enrollment_1.Enrollment.aggregate([
            {
                $match: {
                    classId: { $in: classIds },
                    status: 'active',
    ...
    classIds.forEach((id) => uniqueByClass.set(String(id), new Set()));
    directRows.forEach((row) => {
        const key = String(row._id);
        const bucket = uniqueByClass.get(key) ?? new Set();
        (row.studentIds ?? []).forEach((id) => bucket.add(String(id)));
...
async function uniqueActiveStudentCountForClasses(classIds) {
    if (!classIds.length)
        return 0;
    const [directIds, enrollmentIds] = await Promise.all([
        Student_1.Student.distinct('_id', {
...
function withClassStudentCounts(classes, countMap) {
...
function messageDto(item, viewerId) {
...
function notificationDto(item, lang = 'en', options = {}) {
    const title = (0, localizedText_1.resolveLocalizedText)(item.title, lang);
    const body = (0, localizedText_1.resolveLocalizedText)(item.message ?? item.description, lang);
    ...
    const isRead = Boolean(viewerId && (readBy.includes(viewerId) || item.readAt));
    return {
        id: String(item._id ?? item.id ?? ''),
        _id: String(item._id ?? item.id ?? ''),
        title,
...
function mapAudienceRoles(audience) {
    const value = String(audience ?? '').trim().toLowerCase();
    if (value === 'students' || value === 'student')
        return ['student'];
    if (value === 'parents' || value === 'parent')
        return ['parent'];
    if (value === 'students_and_parents' || value === 'both')
...
async function estimateNotificationRecipients(payload) {
    ...
    if (!classId || !roles.length)
        return estimate;
    if (roles.includes('student') || roles.includes('family_student')) {
        estimate += await Student_1.Student.countDocuments({ classId, status: 'active', isDeleted: false });
    }
    if (roles.includes('parent')) {
        const students = await Student_1.Student.find({ classId, status: 'active', isDeleted: false })
            .select('parentProfileId userId')
            .lean();
        const parentIds = new Set(students
...
async function studentUserForStudentId(studentId) {
    ...
    if (student.studentId)
        orClauses.push({ studentId: student.studentId });
    if (student.loginEmail)
        orClauses.push({ email: String(student.loginEmail).trim().toLowerCase() });
    if (fallbackEmail)
        orClauses.push({ email: fallbackEmail });
    if (!orClauses.length)
        return null;
...
function learningResourceDto(item) {
...
function normalizeLearningResourceType(raw, mime = '', fileName = '') {
    const value = String(raw ?? '').trim().toLowerCase();
    if (LEARNING_RESOURCE_TYPES.has(value))
        return value;
    const combined = `${mime} ${fileName}`.toLowerCase();
    if (combined.includes('video') || combined.includes('.mp4'))
        return 'video';
    if (combined.includes('epub') || combined.includes('book'))
        return 'book';
    return 'document';
}
function sanitizeExternalResourceUrl(raw) {
...
function persistLearningResourceUpload(file) {
    if (!file)
        return null;
    const ext = path_1.default.extname(file.originalname || '').toLowerCase() || '';
    ...
    const finalPath = path_1.default.join(learningResourceUploadRoot, storedName);
...
function resolveLearningResourceFilePath(item) {
    const stored = String(item.fileName || '').trim();
    if (stored) {
        const candidate = path_1.default.resolve(learningResourceUploadRoot, path_1.default.basename(stored));
        if (candidate.startsWith(learningResourceUploadRoot) && fs_1.default.existsSync(candidate)) {
            return candidate;
        }
    }
    const url = String(item.url || '');
    if (url.startsWith('/uploads/learning-resources/')) {
        const candidate = path_1.default.resolve(process.cwd(), url.replace(/^\//, ''));
        if (candidate.startsWith(path_1.default.resolve(process.cwd(), 'uploads')) && fs_1.default.existsSync(candidate)) {
            return candidate;
        }
    }
    return null;
}
async function assertTeacherSubjectForClass(req, classId, subjectIdValue) {
    const subjectId = objectId(subjectIdValue);
    if (!subjectId)
        return null;
    const subject = await Subject_1.Subject.findOne({ _id: subjectId, isDeleted: false }).lean();
    if (!subject)
        return null;
    if (String(subject.classId ?? '') !== String(classId))
        return null;
    return subject;
}
...
exports.mobileRouter.get('/student/dashboard', async (req, res, next) => {
    try {
        ...
        const [classes, exams, attendanceCount, presentCount, notifications, payments] = await Promise.all([
            classId ? Class_1.ClassModel.find({ _id: classId, isDeleted: false }).select('className name title classCode room weeklySchedule studentCount capacity description shortDescription genderRestriction imageUrl thumbnailUrl galleryImages teacherId assignedTeachers subjectId assignedSubjects startDate endDate active registrationOpen').populate('teacherId', 'name firstName lastName').populate('assignedTeachers', 'name firstName lastName').populate('subjectId', 'title name subjectName').populate('assignedSubjects', 'title name subjectName').lean() : Promise.resolve([]),
...
exports.mobileRouter.get('/student/classes', async (req, res, next) => {
    try {
        ...
        const enrollments = studentObjectId
            ? await Enrollment_1.Enrollment.find({ studentId: studentObjectId, isDeleted: false })
                .select('classId')
                .lean()
            : [];
        const classIds = [
        ...
        const classes = uniqueClassIds.length
            ? await Class_1.ClassModel.find({ _id: { $in: uniqueClassIds }, isDeleted: false }).select('className name title classCode room weeklySchedule studentCount capacity description shortDescription genderRestriction imageUrl thumbnailUrl galleryImages teacherId assignedTeachers subjectId assignedSubjects startDate endDate active registrationOpen').populate('teacherId', 'name firstName lastName').populate('assignedTeachers', 'name firstName lastName').populate('subjectId', 'title name subjectName').populate('assignedSubjects', 'title name subjectName').limit(maxItems).lean()
            : [];
...
exports.mobileRouter.get('/student/courses', async (req, res, next) => {
    try {
        const student = await currentStudent(req);
        const studentId = objectId(student?._id);
        if (!studentId) {
        ...
        const enrollments = await Enrollment_1.Enrollment.find({ studentId, isDeleted: false })
        ...
        const filters = [];
        if (classIds.length)
            filters.push({ linkedClassId: { $in: classIds } });
        if (subjectIds.length)
            filters.push({ subjects: { $in: subjectIds } });
        if (teacherIds.length)
            filters.push({ $or: [{ teacher: { $in: teacherIds } }, { instructor: { $in: teacherIds } }] });
        if (!filters.length) {
            ok(res, []);
            return;
        }
...
exports.mobileRouter.get('/student/schedule', async (req, res, next) => {
    try {
        ...
        const [entries, fallbackClass] = await Promise.all([
            Timetable_1.Timetable.find({ classId, isDeleted: false, active: { $ne: false }, isActive: { $ne: false } })
                ...
                .limit(maxItems)
                .lean(),
            Class_1.ClassModel.findById(classId)
        ...
        const fallbackEntries = (fallback?.weeklySchedule ?? []).map((slot) => scheduleDto({ ...slot, room: fallback?.room, classId: fallback }, fallback));
...
exports.mobileRouter.get('/student/attendance', async (req, res, next) => {
    try {
        ...
        const records = studentId
            ? await Attendance_1.Attendance.find({ studentId, isDeleted: false })
...
exports.mobileRouter.get('/student/finance', async (req, res, next) => {
    try {
        ...
        const payments = studentId
            ? await Payment_1.Payment.find({ studentId, isDeleted: false })
...
exports.mobileRouter.get('/student/announcements', async (req, res, next) => {
    try {
        ...
        ].filter(Boolean))).filter(mongoose_1.default.Types.ObjectId.isValid).map((id) => new mongoose_1.default.Types.ObjectId(id));
...
exports.mobileRouter.get('/student/chat/contacts', async (req, res, next) => {
    try {
        ...
        const recentMessages = await Message_1.Message.find({
            isDeleted: false,
            $or: [
        ...
        const decorateContact = (contact) => {
            ...
            const unreadCount = relevant.filter((item) => (item.status === 'unread' &&
                String(item.senderId ?? '') !== String(context.userId) &&
                (contact.role === 'class_group' || String(item.recipientId ?? '') === String(context.userId)))).length;
            return {
                ...contact,
...
exports.mobileRouter.get('/parent/dashboard', async (req, res, next) => {
    try {
        ...
        const classAudience = [];
        if (classIds.length)
            classAudience.push({ classId: { $in: classIds } });
        if (parentUserId)
            classAudience.push({ recipientIds: parentUserId });
        ...
        const announcementAlerts = announcements.map((item) => ({
            ...
            type: 'announcement',
            announcementTitle: (0, localizedText_1.resolveLocalizedText)(item.title, String(req.query.lang ?? 'en'))
        }));
        ok(res, {
...
exports.mobileRouter.get('/teacher/dashboard', async (req, res, next) => {
    try {
        ...
        const assignedClassIds = Array.isArray(teacher?.assignedClassIds)
            ? teacher.assignedClassIds.map(objectId).filter((id) => Boolean(id))
            : [];
        const assignedSubjectIds = Array.isArray(teacher?.assignedSubjectIds)
            ? teacher.assignedSubjectIds.map(objectId).filter((id) => Boolean(id))
            : [];
        const classScope = [
            { teacherId: userId },
        ...
        const classes = await Class_1.ClassModel.find({
            ...
            .limit(maxItems)
            .lean();
        const classIds = classes.map((item) => objectId(item._id)).filter((id) => Boolean(id));
        const classSubjectIds = classes.flatMap((item) => [
            objectId(item.subjectId?._id ?? item.subjectId),
            ...(Array.isArray(item.assignedSubjects)
                ? item.assignedSubjects.map((subject) => objectId(subject?._id ?? subject))
                : [])
        ]).filter((id) => Boolean(id));
        const subjectIds = Array.from(new Map([...assignedSubjectIds, ...classSubjectIds].map((id) => [String(id), id])).values());
        const examScope = [{ teacherId: userId }];
        if (classIds.length)
            examScope.push({ class: { $in: classIds } });
        if (subjectIds.length)
            examScope.push({ subject: { $in: subjectIds } });
        const messageScope = [
            { senderId: userId },
            { recipientId: userId },
            { teacherId: userId },
        ...
        if (classIds.length)
            messageScope.push({ classId: { $in: classIds } });
        ...
        if (classIds.length)
            notificationScope.push({ classId: { $in: classIds } });
        if (subjectIds.length)
            notificationScope.push({ subjectId: { $in: subjectIds } });
        const [assignedStudentsCount, courses, subjects, timetable, attendance, exams, salary, messages, notifications] = await Promise.all([
            (async () => {
            ...
            Course_1.Course.find({
                ...
                .lean(),
            subjectIds.length
                ? Subject_1.Subject.find({ _id: { $in: subjectIds }, isDeleted: false })
                    .select('title name subjectName')
                    .limit(maxItems)
                    .lean()
                : Promise.resolve([]),
            Timetable_1.Timetable.find({ teacherId: userId, isDeleted: false, active: { $ne: false }, isActive: { $ne: false } })
            ...
            Attendance_1.Attendance.find({
                ...
                .limit(500)
                .lean(),
            Exam_1.Exam.find({ isDeleted: false, $or: examScope })
                .select('title date totalMarks passingMarks subject class teacherId status')
                .sort({ date: -1 })
                .limit(maxItems)
                .lean(),
            SalaryRecord_1.SalaryRecord.findOne({
                userId,
                role: 'teacher',
                isDeleted: false
            })
                ...
                .lean(),
            Message_1.Message.find({ isDeleted: false, $or: messageScope })
        ...
        const examIds = exams.map((item) => objectId(item._id)).filter((id) => Boolean(id));
        const [examResults, legacyResults] = await Promise.all([
            examIds.length
                ? ExamResult_1.ExamResult.find({ examId: { $in: examIds }, isDeleted: false })
                    .select('studentId examId subjectId score grade passed publishedAt')
                    .limit(500)
                    .lean()
                : Promise.resolve([]),
            Result_1.Result.find({
                isDeleted: false,
                $or: [
                    { teacherId: userId },
                    ...(classIds.length ? [{ classId: { $in: classIds } }] : []),
        ...
        const subjectName = (id) => {
            ...
            return (0, localizedText_1.resolveLocalizedText)(subject?.title ?? subject?.name ?? subject?.subjectName, lang);
        };
        ...
        const highlights = [
            ...
            ...recentNotifications.map((notification) => ({
                ...
                title: (0, localizedText_1.resolveLocalizedText)(notification.title, lang),
                message: (0, localizedText_1.resolveLocalizedText)(notification.message ?? notification.description, lang),
        ...
        const branch = branchId
            ? await Branch_1.Branch.findById(branchId).select('name code').lean()
            : null;
        ok(res, {
            ...
            pendingAttendance,
            pendingGrades,
            monthlySalaryAf: Number(salary?.netSalary ??
                salary?.netAmount ??
                teacher?.fixedSalary ??
                user?.fixedSalary ??
                0),
            ...
            salaryPaymentStatus: (() => {
                ...
                if (status === 'paid' || (net > 0 && paid >= net - 0.009))
                    return 'paid';
                if (paid > 0 && paid < net - 0.009)
            ...
            salaryRemainingAmountAf: Number(Math.max(0, Number(salary?.netSalary ?? salary?.netAmount ?? 0) - Number(salary?.paidAmount ?? 0)).toFixed(2)),
            highlights,
            analytics,
            summary: {
...
exports.mobileRouter.get('/teacher/classes', async (req, res, next) => {
    try {
        ...
        const classes = await Class_1.ClassModel.find({ $or: classScope, isDeleted: false })
        ...
        ok(res, withClassStudentCounts(classes, countMap).map((item) => classDto(item, { timetableByClass })));
    }
    catch (error) {
        next(error);
    }
});
exports.mobileRouter.get('/teacher/courses', async (req, res, next) => {
    try {
        ...
        const [teacherClasses, teacherSubjects] = await Promise.all([
            Class_1.ClassModel.find({
                ...
                .populate('assignedSubjects', 'title name subjectName')
                .lean(),
            Subject_1.Subject.find({ teacher: teacherObjectId, isDeleted: false })
        ...
        teacherClasses.forEach((klass) => {
            ...
            if (primarySubjectId)
                subjectIdSet.add(String(primarySubjectId));
            (Array.isArray(klass.assignedSubjects) ? klass.assignedSubjects : []).forEach((subject) => {
        ...
        if (classIds.length)
            courseFilters.push({ linkedClassId: { $in: classIds } });
        if (subjectIds.length)
            courseFilters.push({ subjects: { $in: subjectIds } });
        const courses = await Course_1.Course.find({
            isDeleted: false,
            status: { $ne: 'archived' },
            $or: courseFilters
        ...
        const payload = await Promise.all(courses.map(async (course) => {
            const courseSubjectIds = new Set((Array.isArray(course.subjects) ? course.subjects : []).map((subject) => String(subject?._id ?? subject)));
            ...
            const relatedClasses = teacherClasses
                ...
                const classSubjectIds = [
                    ...
                    ...(Array.isArray(klass.assignedSubjects) ? klass.assignedSubjects : []).map((subject) => String(subject?._id ?? subject))
                ].filter(Boolean);
            ...
            const relatedSubjects = teacherSubjects.filter((subject) => courseSubjectIds.has(String(subject._id)));
            const relatedClassObjectIds = relatedClasses
                .map((klass) => objectId(klass._id))
                .filter((id) => Boolean(id));
...
exports.mobileRouter.get('/teacher/parent-contacts', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId)
            return ok(res, []);
        const classIds = await teacherAssignedClassIds(req.user?.userId);
        const studentScope = [{ teacherId }];
        if (classIds.length)
            studentScope.push({ classId: { $in: classIds } });
        ...
        const parents = parentIds.length || studentObjectIds.length
            ...
            }).select('userId guardianName guardianPhone guardianEmail relationType linkedStudentIds').lean()
            : [];
        const parentsById = new Map(parents.map((parent) => [String(parent._id), parent]));
        const parentsByStudentId = new Map();
        parents.forEach((parent) => {
...
exports.mobileRouter.get('/teacher/students/:studentId/parent-contact', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        ...
        const populated = await Student_1.Student.findById(student._id)
            .select('firstName lastName studentId classId parentProfileId familyPhone whatsapp fatherName')
            .populate('classId', 'className name classCode')
            .lean();
        if (!populated) {
        ...
        const parent = populated.parentProfileId
            ? await Parent_1.ParentProfile.findOne({
...
exports.mobileRouter.get('/teacher/parents/:parentId/whatsapp-qr', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        ...
        const parent = await Parent_1.ParentProfile.findOne({
            _id: parentId,
            isDeleted: false
        }).select('guardianName guardianPhone guardianEmail linkedStudentIds').lean();
        if (!parent) {
        ...
        const linkedStudentIds = [
            ...
            ...(await Student_1.Student.find({
                ...
                isDeleted: false
            }).select('_id').lean()).map((item) => objectId(item._id)).filter((id) => Boolean(id))
        ];
...
exports.mobileRouter.get('/teacher/resources', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId) {
            return res.status(401).json({ success: false, message: 'Authentication required' });
        }
        const classFilter = await Class_1.ClassModel.find({
            isDeleted: false,
        ...
        const classIds = classFilter
            .map((item) => objectId(item._id))
            .filter((id) => Boolean(id));
        const records = await LearningResource_1.LearningResource.find({
            isDeleted: false,
            $or: [
...
exports.mobileRouter.get('/teacher/resources/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        ...
        const item = await LearningResource_1.LearningResource.findOne({ _id: resourceId, isDeleted: false })
            .populate('classId', 'className name')
            .populate('subjectId', 'title name subjectName')
            .populate('uploadedBy', 'name email')
            .lean();
...
exports.mobileRouter.post('/teacher/resources', (req, res, next) => {
    ...
    try {
        ...
        const type = normalizeLearningResourceType(req.body?.type ?? req.body?.resourceType, req.file?.mimetype, req.file?.originalname);
        if (!title) {
        ...
        const resource = await LearningResource_1.LearningResource.create({
            title,
            description,
            type,
            classId,
        ...
        const populated = await LearningResource_1.LearningResource.findById(resource._id)
            .populate('classId', 'className name')
            .populate('subjectId', 'title name subjectName')
            .populate('uploadedBy', 'name email')
            .lean();
...
exports.mobileRouter.patch('/teacher/resources/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const resourceId = objectId(req.params.id);
        if (!teacherId || !resourceId) {
        ...
        const existing = await LearningResource_1.LearningResource.findOne({
            _id: resourceId,
            uploadedBy: teacherId,
            isDeleted: false
        }).lean();
        ...
        const updated = await LearningResource_1.LearningResource.findByIdAndUpdate(resourceId, { $set: updates }, { new: true, runValidators: true })
            .populate('classId', 'className name')
            .populate('subjectId', 'title name subjectName')
            .populate('uploadedBy', 'name email')
            .lean();
...
exports.mobileRouter.delete('/teacher/resources/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const resourceId = objectId(req.params.id);
        if (!teacherId || !resourceId) {
        ...
        const existing = await LearningResource_1.LearningResource.findOneAndUpdate({ _id: resourceId, uploadedBy: teacherId, isDeleted: false }, { $set: { isDeleted: true, deletedAt: new Date(), deletedBy: teacherId, published: false } }, { new: true }).lean();
        if (!existing) {
            return res.status(404).json({ success: false, message: 'Resource not found' });
        }
...
exports.mobileRouter.get('/teacher/resources/:id/download', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const resourceId = objectId(req.params.id);
        if (!teacherId || !resourceId) {
        ...
        const item = await LearningResource_1.LearningResource.findOne({ _id: resourceId, isDeleted: false }).lean();
        if (!item)
            return res.status(404).json({ success: false, message: 'Resource not found' });
        ...
        res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(item.fileOriginalName || item.fileName || 'resource')}"`);
...
exports.mobileRouter.get('/student/resources', async (req, res, next) => {
    try {
        const context = await currentStudentContext(req);
        ...
        const classIds = Array.from(new Set([
            ...
            .filter(mongoose_1.default.Types.ObjectId.isValid)
            .map((id) => new mongoose_1.default.Types.ObjectId(id));
...
exports.mobileRouter.get('/student/resources/:id/download', async (req, res, next) => {
    try {
        const context = await currentStudentContext(req);
        const resourceId = objectId(req.params.id);
        ...
        const item = await LearningResource_1.LearningResource.findOne({
            _id: resourceId,
            published: true,
            isDeleted: false
        }).lean();
...
exports.mobileRouter.get('/teacher/schedule', async (req, res, next) => {
    try {
        ...
        const timetableEntries = classIds.length
            ? await Timetable_1.Timetable.find({
                classId: { $in: classIds },
                teacherId: teacherObjectId,
                isDeleted: false,
                active: { $ne: false },
                ...
                .lean()
            : [];
        const classesWithTimetable = new Set(timetableEntries.map((entry) => String(entry.classId?._id ?? entry.classId ?? '')));
        const teacherIdKey = String(teacherObjectId);
        const fallbackSlots = classes.flatMap((item) => {
            const classId = String(item._id ?? '');
            if (!classId || classesWithTimetable.has(classId))
                return [];
...
function isValidObjectId(value) {
    return mongoose_1.default.Types.ObjectId.isValid(String(value ?? '')) &&
        String(value).length === 24;
}
async function teacherAssignedClassIds(userId) {
...
function attendanceDateRangeFromQuery(req) {
    if (req.query.date) {
        return {
...
async function buildTeacherAttendanceFilter(req) {
    const userId = req.user?.userId;
    const teacherObjectId = objectId(userId);
...
exports.mobileRouter.post('/teacher/attendance/register', async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(403).json({ success: false, message: 'Permission denied' });
        ...
        const timetableEntries = await Timetable_1.Timetable.find({
            classId: classDoc._id,
            isDeleted: false,
            isActive: { $ne: false },
            active: { $ne: false },
        ...
        let activeTimetable = windowEntries.find((entry) => String(entry.teacherId ?? '') === String(teacherObjectId)) ??
            windowEntries.find((entry) => !entry.teacherId) ??
            windowEntries[0] ??
            null;
        if (!activeTimetable && timetableEntries.length === 0) {
            const todayDow = kabulWeekday(now);
            const weekly = resolveClassScheduleSlots(classDoc).filter((slot) => Number(slot.dayOfWeek) === todayDow);
        ...
        const existing = await Attendance_1.Attendance.findOne(duplicateFilter).sort({ updatedAt: -1 });
        ...
        try {
            saved = existing
                ? await Attendance_1.Attendance.findByIdAndUpdate(existing._id, { $set: payload }, { new: true, runValidators: true }).lean()
                : await Attendance_1.Attendance.create(payload).then((doc) => doc.toObject());
        }
        catch (error) {
            if (error?.code === 11000) {
                const conflict = await Attendance_1.Attendance.findOne(duplicateFilter).sort({ updatedAt: -1 });
                if (conflict) {
                    saved = await Attendance_1.Attendance.findByIdAndUpdate(conflict._id, { $set: payload }, { new: true, runValidators: true }).lean();
                }
                else {
                    throw error;
                }
...
exports.mobileRouter.get('/teacher/attendance', async (req, res, next) => {
    try {
        ...
        const limit = Math.min(Math.max(Number(req.query.limit ?? maxItems) || maxItems, 1), 500);
        const records = await Attendance_1.Attendance.find(filter)
...
exports.mobileRouter.get('/teacher/salary', async (req, res, next) => {
    try {
        ...
        const { year: currentHijriYear, month: currentHijriMonth } = (0, afghanistanSalaryTaxService_1.getHijriYearMonth)();
        ...
        if (period === 'this_month') {
            ...
            if (queryMonth > 0)
                recordFilter.hijriMonth = queryMonth;
        }
        ...
        if (hasValidFrom || hasValidTo) {
            ...
            if (hasValidFrom)
                recordFilter.calculatedAt.$gte = fromDate;
            if (hasValidTo) {
        ...
        const [payrollRecords, payoutRecords, paymentRecords] = await Promise.all([
            SalaryRecord_1.SalaryRecord.find(recordFilter)
                .populate('userId', 'name role email')
                .populate('branchId', 'name code')
                .sort({ hijriYear: -1, hijriMonth: -1, calculatedAt: -1 })
                .limit(maxItems)
                .lean(),
            Salary_1.Salary.find({
                employeeId: userId,
                isDeleted: false
            })
        ...
        const payoutByMonth = new Map(payoutRecords.map((item) => [String(item.monthKey ?? ''), item]));
        const paymentMatchesPeriod = (item) => {
            ...
            if (period === 'this_year') {
                if (linkedPeriod)
                    return linkedPeriod.startsWith(`${currentHijriYear}-`);
                return Boolean(paymentDate &&
                    !Number.isNaN(paymentDate.getTime()) &&
                    paymentDate.getFullYear() === new Date().getFullYear());
            }
            if (hasValidFrom || hasValidTo) {
                if (!paymentDate || Number.isNaN(paymentDate.getTime()))
                    return false;
                if (hasValidFrom && paymentDate < fromDate)
                    return false;
                if (hasValidTo) {
                    const end = new Date(toDate);
                    end.setHours(23, 59, 59, 999);
        ...
        const mapPaymentStatus = (net, paid, rawStatus) => {
        ...
        const summaryStatus = mapPaymentStatus(totalNet, totalPaid, current?.paymentStatus ?? 'unpaid');
...
exports.mobileRouter.get('/teacher/salary/payments/:paymentId/receipt', async (req, res, next) => {
    try {
        const userId = objectId(req.user?.userId);
...
exports.mobileRouter.get('/teacher/exams', async (req, res, next) => {
    try {
        const exams = await Exam_1.Exam.find({ teacherId: req.user?.userId, isDeleted: false })
...
exports.mobileRouter.get('/teacher/exams/:examId/analysis', async (req, res, next) => {
    try {
        ...
        const insight = await (0, aiResultAnalysis_service_1.generateExamInsights)(req.params.examId);
        ok(res, {
            ...insight,
...
exports.mobileRouter.get('/teacher/grading', async (req, res, next) => {
    try {
        ...
        const teacherExamIds = await Exam_1.Exam.find(teacherExamFilter).select('_id').lean();
...
exports.mobileRouter.get('/teacher/results/:resultId/suggestion', async (req, res, next) => {
    try {
        const language = String(req.query.lang || req.body?.lang || 'en');
        const result = await Result_1.Result.findById(req.params.resultId)
        ...
        const [learningResources, books] = await Promise.all([
            LearningResource_1.LearningResource.find(resourceFilter).select('title type url description').sort({ updatedAt: -1 }).limit(5).lean(),
            Book_1.Book.find(subjectId
                ? { available: true, isDeleted: false, title: { $regex: String(result.exam?.subject?.title || ''), $options: 'i' } }
                : { available: true, isDeleted: false }).select('title author category').sort({ updatedAt: -1 }).limit(3).lean()
        ]);
        const resources = [
...
exports.mobileRouter.get('/teacher/results/:resultId/insight', async (req, res, next) => {
    try {
        const result = await Result_1.Result.findById(req.params.resultId)
...
async function teacherCanAccessStudent(teacherId, studentDocId) {
    const student = await Student_1.Student.findOne({
        _id: studentDocId,
        isDeleted: false,
        status: { $ne: 'inactive' }
    ...
    if (enrollment || classDoc || timetableEntry)
        return student;
    return null;
}
async function resolveTeacherStudentChat(req, studentDocIdValue) {
    const teacherId = objectId(req.user?.userId);
...
async function normalizeAfghanWhatsappPhone(phone) {
    const cleaned = sanitizePhoneInput(phone);
    const digits = cleaned.replace(/\D/g, '');
    const normalized = digits.startsWith('00')
        ? digits.slice(2)
...
exports.mobileRouter.get('/teacher/students/:studentId/whatsapp-qr', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const studentDocId = objectId(req.params.studentId);
        if (!teacherId || !studentDocId) {
        ...
        const phone = String(student.whatsapp ?? student.familyPhone ?? student.phone ?? '').trim();
...
exports.mobileRouter.get('/teacher/chat/contacts', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId)
            return ok(res, { students: [], parents: [], admins: [] });
        const classes = await Class_1.ClassModel.find({
            isDeleted: false,
            $or: [{ teacherId }, { assignedTeachers: teacherId }]
        }).select('_id').lean();
        ...
        const parents = parentIds.length
            ? await Parent_1.ParentProfile.find({ _id: { $in: parentIds }, isDeleted: false })
        ...
        const recentMessages = await Message_1.Message.find({
            ...
            .limit(400)
            .lean();
        const decorate = (contact, matchIds) => {
            ...
            const unreadCount = relevant.filter((item) => (item.status === 'unread' &&
                ...
                String(item.recipientId ?? '') === String(teacherId))).length;
            return {
                ...contact,
                lastMessage: last?.body ?? '',
                timestamp: compactDate(last?.createdAt),
...
exports.mobileRouter.get('/teacher/chat/messages', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId)
            return ok(res, []);
...
exports.mobileRouter.post('/teacher/chat/messages', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId) {
        ...
        if (targetType === 'student') {
            ...
            const scope = resolved.scope;
            const user = await currentUser(req);
            const existingThread = await Message_1.Message.findOne({
                isDeleted: false,
                $or: [
                    { senderId: scope.teacherId, recipientId: scope.studentUserId },
                    { senderId: scope.studentUserId, recipientId: scope.teacherId }
...
exports.mobileRouter.get('/teacher/chat/students/:studentId/messages', async (req, res, next) => {
    try {
...
exports.mobileRouter.post('/teacher/chat/students/:studentId/messages', async (req, res, next) => {
    try {
        const resolved = await resolveTeacherStudentChat(req, req.params.studentId);
        if (!resolved.scope) {
            const status = resolved.error === 'account_not_linked' ? 404 : 403;
...
exports.mobileRouter.get('/teacher/notifications', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId) {
            return res.status(401).json({ success: false, message: 'Authentication required' });
        ...
        const records = await Notification_1.Notification.find({
            isDeleted: false,
            ...branchFilter(req),
            $or: [
                { teacherId },
        ...
        ok(res, records.map((item) => {
            ...
            const roles = Array.isArray(item.recipientRoles)
                ? item.recipientRoles.map((role) => String(role).toLowerCase())
                : [];
            const targetsStudentsOrParents = roles.some((role) => role === 'student' || role === 'parent' || role === 'family_student');
            const createdVia = String(item.metadata?.createdVia ?? '');
            const owned = teacherRef === String(teacherId) && (targetsStudentsOrParents ||
            ...
            const read = Boolean(Array.isArray(item.readBy) &&
                item.readBy.some((id) => String(id) === String(req.user?.userId)));
            return {
                ...dto,
                status: owned
...
exports.mobileRouter.get('/teacher/notifications/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        ...
        const item = await Notification_1.Notification.findOne({
            _id: notificationId,
            isDeleted: false,
            ...branchFilter(req),
            $or: [
        ...
        const roles = Array.isArray(item.recipientRoles)
            ...
            : [];
        const targetsStudentsOrParents = roles.some((role) => role === 'student' || role === 'parent' || role === 'family_student');
        const isAuthor = teacherOwnsRecord && (targetsStudentsOrParents ||
        ...
        const recipientEstimate = isAuthor
            ...
            : 0;
        const dto = notificationDto({ ...item, recipientEstimate }, lang, { viewerId: String(req.user?.userId ?? ''), owned: isAuthor });
        ok(res, {
            ...dto,
...
exports.mobileRouter.post('/teacher/notifications', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        if (!teacherId) {
            return res.status(401).json({ success: false, message: 'Authentication required' });
        ...
        const title = String(req.body?.title ?? '').trim();
        const description = String(req.body?.description ?? req.body?.message ?? req.body?.content ?? '').trim();
        const classId = objectId(req.body?.classId);
        const subjectId = objectId(req.body?.subjectId);
        const audienceRoles = mapAudienceRoles(req.body?.audience ?? req.body?.audienceType ?? req.body?.recipientRoles);
        ...
        res.status(201).json({
            success: true,
            data: {
                ...notificationDto({ ...(populated ?? notification.toObject()), recipientEstimate }, lang, { viewerId: String(teacherId), owned: true }),
                totalRecipients: recipientEstimate
            },
...
exports.mobileRouter.patch('/teacher/notifications/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const notificationId = objectId(req.params.id);
        if (!teacherId || !notificationId) {
        ...
        if (req.body?.classId !== undefined || req.body?.audience !== undefined || req.body?.audienceType !== undefined || req.body?.recipientRoles !== undefined) {
            ...
            const audienceRoles = mapAudienceRoles(req.body?.audience ?? req.body?.audienceType ?? req.body?.recipientRoles ?? existing.recipientRoles);
            if (!audienceRoles.length) {
                return res.status(400).json({ success: false, message: 'Audience is required' });
            }
            updates.classId = classId;
        ...
        const updated = await Notification_1.Notification.findByIdAndUpdate(notificationId, { $set: updates }, { new: true, runValidators: true })
            .populate('classId', 'className name classCode')
            .populate('subjectId', 'title name subjectName')
            .populate('teacherId', 'name email')
            .lean();
...
exports.mobileRouter.delete('/teacher/notifications/:id', async (req, res, next) => {
    try {
        const teacherId = objectId(req.user?.userId);
        const notificationId = objectId(req.params.id);
        if (!teacherId || !notificationId) {
        ...
        const existing = await Notification_1.Notification.findOneAndUpdate({ _id: notificationId, teacherId, isDeleted: false }, { $set: { isDeleted: true, deletedAt: new Date(), deletedBy: teacherId } }, { new: true }).lean();
        if (!existing) {
            return res.status(404).json({ success: false, message: 'Announcement not found' });
        }
        ok(res, { id: String(existing._id), deleted: true });
