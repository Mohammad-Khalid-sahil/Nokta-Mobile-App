...
function serializeSubject(subject, extras = {}) {
...
function formatScheduleSlot(entry) {
...
async function enrichSubjectListItems(subjects, language = 'en') {
    if (!subjects.length)
        return subjects;
    ...
    const [courses, timetables, classes] = await Promise.all([
        subjectIds.length
            ? Course_1.Course.find({ subjects: { $in: subjectIds }, isDeleted: false })
                .select('title subjects imageUrl')
                .lean()
            : Promise.resolve([]),
        subjectIds.length
            ? Timetable_1.Timetable.find({ subjectId: { $in: subjectIds }, isDeleted: false })
                .select('subjectId dayOfWeek startTime endTime room')
                .sort({ dayOfWeek: 1, startTime: 1 })
                .lean()
            : Promise.resolve([]),
        classIds.length
            ? Class_1.ClassModel.find({ _id: { $in: classIds }, isDeleted: false })
    ...
    return subjects.map((subject) => {
        ...
        const extraClassNames = (subject?.classIds ?? [])
            .map((classId) => classById.get(String(classId?._id ?? classId)))
            .filter(Boolean)
        ...
        return serializeSubject(subject, {
            ...
            courseName: course?.title ? (0, localizedText_1.resolveLocalizedText)(course.title, language) : '',
...
router.get('/', (0, auth_1.authorize)(['super_admin', 'admin', 'branch_manager', 'teacher', 'student', 'family_student', 'parent', 'owner']), (0, validate_1.validate)(subjectQuerySchema), async (req, res, next) => {
    try {
        ...
        res.json((0, response_1.createResponse)(enrichedSubjects, '', { page, limit, total }));
    }
    catch (error) {
        next(error);
    }
