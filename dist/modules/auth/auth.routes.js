...
router.post('/register/student', rateLimiter_1.authLimiter, async (req, res) => {
    try {
        ...
        const result = await publicRegistrationService.registerStudent({
            ...
            nationalId: (0, inputSecurity_1.sanitizePlainText)(value.nationalId, 80),
...
router.put('/profile', auth_1.authenticate, (0, validate_1.validate)(profileUpdateSchema), async (req, res) => {
    try {
        ...
        if (Object.keys(payload).length === 0) {
            return res.status(400).json((0, response_1.createError)('No profile fields to update'));
        }
        const user = await User_1.User.findOneAndUpdate({ _id: req.user.userId, isDeleted: false }, { $set: payload }, { new: true, runValidators: true }).select('-password').lean();
        if (!user)
