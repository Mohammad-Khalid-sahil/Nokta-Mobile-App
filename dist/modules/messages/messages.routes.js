        ...
        if (role === 'student') {
            ...
            if (requestedRecipientRole === 'student' && body.studentId) {
                const student = await Student_1.Student.findOne({
                    ...
                    teacherId: userId,
                    isDeleted: false
                }).select('_id branchId classId studentId loginEmail').lean();
                if (!student)
                    return res.status(403).json((0, response_1.createError)('Student is not assigned to this teacher'));
                ...
                if (!studentUser?._id) {
                    return res.status(404).json((0, response_1.createError)('Student account was not found'));
                }
                ...
                const student = await Student_1.Student.findOne({
                    ...
                    teacherId: userId,
                    isDeleted: false
                }).select('_id branchId parentProfileId').lean();
                if (!student)
                    return res.status(403).json((0, response_1.createError)('Parent is not linked to your students'));
