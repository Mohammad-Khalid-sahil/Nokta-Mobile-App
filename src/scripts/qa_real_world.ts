...
const credentials: Record<string, LoginCred> = {
...
async function http(path: string, token?: string, init: RequestInit = {}): Promise<HttpResult> {
...
async function main() {
  ...
  const closedReportRow = reportSessions.find((item: any) => String(item.timetableId) === String(closed.timetableId));
  ...
  if (created.course) await http(`/courses/${created.course}`, adminToken, { method: 'DELETE' });
  if (created.subject) await http(`/subjects/${created.subject}`, adminToken, { method: 'DELETE' });
  if (created.class) await http(`/classes/${created.class}`, adminToken, { method: 'DELETE' });
