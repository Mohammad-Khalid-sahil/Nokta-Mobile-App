...
const mobileCache = new Map<string, { expiresAt: number; body: unknown; statusCode: number }>();
const mobileCacheTtlMs = Number(process.env.MOBILE_RESPONSE_CACHE_TTL_MS ?? 8000);

function isCacheableMobileGet(req: Request) {
...
export function mobilePerformanceMiddleware(req: Request, res: Response, next: NextFunction) {
  const startedAt = Date.now();

  res.setHeader('X-Backend-Node', 'nokta-academy');
  const originalJson = res.json.bind(res);

  function sendJson(body: unknown) {
