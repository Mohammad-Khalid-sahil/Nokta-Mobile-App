export type DashboardPeriod = 'day' | 'month' | 'quarter' | 'year';
...
export function getDashboardDateRange(period: DashboardPeriod, now = new Date()) {
...
export function buildTrendBuckets(period: DashboardPeriod, range: { start: Date; end: Date }): TrendBucket[] {
...
export function matchTrendBucket(
  period: DashboardPeriod,
  bucket: TrendBucket,
  parts: { year: number; month: number; day?: number; hour?: number }
) {
  if (parts.year !== bucket.year || parts.month !== bucket.month) return false;
  if (period === 'day') return parts.hour === bucket.hour;
  if (period === 'month') return parts.day === bucket.day;
  return true;
}
