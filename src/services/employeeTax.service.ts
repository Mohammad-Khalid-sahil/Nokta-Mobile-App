...
import { calculateSalaryRecord } from './payrollCalculation.service';

function canEditTaxRules(req: any) {
  return req.user?.canonicalRole === 'super_admin' || req.user?.role === 'super_admin';
}

export function getPayrollFilters(req: Request) {
  ...
  const branchId = req.user && (req as any).user?.canonicalRole === 'branch_manager' && (req as any).user?.branchId
    ? String((req as any).user.branchId)
    : req.query.branchId
...
function employeeRoleFilter(role: string) {
...
async function getActivePayrollEmployees(args: { role: string; branchId?: string }) {
...
function getFallbackSalarySetting(employee: any) {
  ...
  if (!hasFixed && !hasPercentage) return null;
...
function normalizeTaxRecord(record: any, employee: any) {
...
export async function buildEmployeeTaxReport(req: any, forceRecalculate = false) {
  ...
  for (const employee of employees) {
    ...
    const hasRealSalarySource = setting && (
      Number(setting.fixedAmount || 0) > 0
    ...
    if (!filteredMonthlyRecords.length) continue;
